...
describe('Security Headers Module', () => {
    test('should apply Content-Security-Policy header', () => {
        ...
        const createdElements = document.createElement.mock.results.map(result => result.value);
    ...
    test('should generate nonces for scripts and styles', () => {
        // Get nonces
        const scriptNonce = securityHeaders_1.securityHeaders.getNonce('script');
        const styleNonce = securityHeaders_1.securityHeaders.getNonce('style');
    ...
    test('should return consistent nonce for the same type', () => {
        ...
        const scriptNonce1 = securityHeaders_1.securityHeaders.getNonce('script');
        const scriptNonce2 = securityHeaders_1.securityHeaders.getNonce('script');
        // Should be the same
        expect(scriptNonce1).toBe(scriptNonce2);
    });
    test('should allow config updates', () => {
        // Update config with partial configuration
        securityHeaders_1.securityHeaders.updateConfig({
            csp: {
        ...
        const config = securityHeaders_1.securityHeaders.getConfig();
    ...
    test('should apply X-Content-Type-Options header', () => {
        ...
        const contentTypeOptionsElement = createdElements.find(el => el.httpEquiv === 'X-Content-Type-Options');
